...
//超出菜单折叠
$.fn.menuMore = function(){

	var tempWidth = 0,
	    menuWidth = $(this).find('.menuOoverallWidth').outerWidth(true) - $(this).find('.menuMore').outerWidth(true),
	    maxWidth = $(this).find('.menuOoverallWidth').outerWidth(true);


	var $moreItems = $(this).find('.sf-menu > li').not('.menuMore').filter(function() {

			tempWidth += $(this).outerWidth(true);
			if (tempWidth >= menuWidth) return true;
			else return false;

	});

	
	if ($moreItems.length > 0 && tempWidth > maxWidth) {
	    $(this).find('.menuMore').show().find('ul').append($moreItems);
	} else {
